...
ofVec2f interpolate(const Controler& controler, const ofVec2f &p)
{
    ...
    ofVec2f tileSize((float)windowWidth/fieldWidth, (float)windowHeight/fieldHeight);

    int index_x = (int)(relative_pos.x * fieldWidth);
    int index_y = (int)(relative_pos.y * fieldHeight);

    // saftey :)
    index_x = MAX(0, MIN(index_x, fieldWidth-1));
    index_y = MAX(0, MIN(index_y, fieldHeight-1));
    ...
    //no wrap;
//    int index_x2 = index_x + 1;
//    int index_y2 = index_y + 1;
